...
+ 记|x|表示 x 的 2-范数/Euclid-Norm，原高维空间计算相似度的函数为 $ S(x_i, x_j) = exp(-\midx_i - x_j\mid) $ 即高斯核/径向基函数，在低维空间 SNE 采用的还是高斯核 $ S'(z_i, z_j) = exp(-\midz_i - z_j\mid) $，而 t-SNE 则利用了 t 分布，其计算相似度的函数为 $ S'(z_i, z_j) = 1 / (1 + \midz_i - z_j\mid) $
